...
public class ExeclTest {  
	 ...
	 private static Logger logger = LoggerFactory.getLogger(ExeclTest.class) ;
	...
	// }  
	 ...
	 public ExeclTest(HSSFWorkbook workbook){  
	     this.workbook = workbook;  
	 }  
	   
	 public ExeclTest(HSSFWorkbook workbook, HSSFSheet sheet) {  
	  super();  
	  this.workbook = workbook;  
	  this.sheet = sheet;  
	 }  
	...
	//         comment.setAuthor("Xuys.");  
	 ...
	 public static void exportExecl(boolean isLandscape ,short pageSize,String fileName,Double[] margins){
		 HSSFWorkbook workbook = new HSSFWorkbook();  
			...
			// 实施自定义,使用打印机默认打印纸
			ps.setPaperSize(pageSize) ; // 纸张类型
			// 打印比例
			ps.setScale((short) 100) ;
			
			sheet.setMargin(HSSFSheet.TopMargin, 0.0787) ;// 页边距（上）
			sheet.setMargin(HSSFSheet.BottomMargin, margins[1]*1.00/ 100) ;// 页边距（下）
			sheet.setMargin(HSSFSheet.LeftMargin, margins[2]*1.00 / 100) ;// 页边距（左）
			sheet.setMargin(HSSFSheet.RightMargin, margins[3]*1.00/ 100) ;// 页边距（右
			logger.debug("====打印页边距 top={},bottom={},left={},right={},fileName={}",sheet.getMargin(HSSFSheet.TopMargin),sheet.getMargin(HSSFSheet.BottomMargin),
					sheet.getMargin(HSSFSheet.LeftMargin),sheet.getMargin(HSSFSheet.RightMargin),fileName);
			
			ExeclTest eeu = new ExeclTest(workbook, sheet);  
		  ...
		  eeu.exportExcel("d://temp//"+fileName+".xls");  
	 }
	   
	 public static void main(String[] args) {  
		 exportExecl(true,HSSFPrintSetup.A4_PAPERSIZE,"test_land_a4",new Double[]{0d,20d,30d,40d});
		 exportExecl(true,HSSFPrintSetup.A3_PAPERSIZE,"test_unland_a3",new Double[]{50d,60d,70d,80d});
	 }  
